...
from src.x_api_client import fetch_tweets, log_rate_limit_info
...
class TestXApiClient:
    ...
    def test_fetch_tweets_success_without_since_id(self):
        ...
        mock_response = {
            "data": [{"id": "123", "text": "Test tweet", "author_id": "user1"}],
            "includes": {"users": [{"id": "user1", "username": "testuser"}]},
        }

        responses.add(responses.GET, "https://api.x.com/2/tweets/search/recent", json=mock_response, status=200)

        with (
            patch("src.x_api_client.get_x_api_headers", return_value={"Authorization": "Bearer test"}),
            patch("src.x_api_client.get_x_api_params", return_value={"query": "test", "max_results": 50}),
        ):
            result = fetch_tweets()
    ...
    def test_fetch_tweets_success_with_since_id(self):
        ...
        mock_response = {"data": [{"id": "124", "text": "New tweet", "author_id": "user1"}]}
    ...
    def test_fetch_tweets_rate_limit(self):
        """レート制限時のテスト"""
        error_response = {"errors": [{"code": 88, "message": "Rate limit exceeded"}]}
        ...
        responses.add(
            ...
            json=error_response,
            status=429,
            headers={"x-rate-limit-limit": "300", "x-rate-limit-remaining": "0", "x-rate-limit-reset": "1640995200"},
        )
    ...
    def test_fetch_tweets_http_error(self):
        """HTTPエラー時のテスト"""
        responses.add(responses.GET, "https://api.x.com/2/tweets/search/recent", json={"error": "Unauthorized"}, status=401)
    ...
    def test_fetch_tweets_connection_error(self):
        """接続エラー時のテスト"""
        responses.add(responses.GET, "https://api.x.com/2/tweets/search/recent", body=ConnectionError("Connection failed"))
