...
def _load_since_id_from_secret_manager():
    ...
    try:
        ...
        since_id = response.payload.data.decode(
            "UTF-8"
        ).strip()

        if since_id:
            logger.info(
                f"Secret Manager から前回の処理IDを読み込み: {since_id}"
            )
            return since_id
        else:
            logger.info(
                "Secret Manager から空の処理IDを読み込み。初回実行として処理します"
            )
            return None
    except Exception as e:
        logger.info(
            f"Secret Manager からの読み込みに失敗: {e}"
        )
        return None
...
def _save_since_id_to_secret_manager(since_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Secret Manager にシークレット {secret_id} を作成しました"
            )
        ...
        response = client.add_secret_version(
            request={
                ...
                "payload": {
                    "data": since_id.encode("UTF-8")
                },
            }
        )
        logger.info(
            f"Secret Manager に処理IDを保存: {since_id} (version: {response.name})"
        )
...
def _load_since_id_from_file():
    ...
    try:
        with open(STATE_FILE, "r", encoding="utf-8") as f:
            ...
            if since_id:
                logger.info(
                    f"ファイルから前回の処理IDを読み込み: {since_id}"
                )
            else:
                logger.info(
                    "ファイルから空の処理IDを読み込み。初回実行として処理します"
                )
            return since_id
...
def save_since_id(since_id: str):
    ...
    if PROJECT_ID and os.getenv(
        ...
        try:
            ...
            logger.warning(
                f"Secret Manager への保存に失敗。ファイルにフォールバック: {e}"
            )
